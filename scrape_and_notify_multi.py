#!/usr/bin/env python3
"""
Finances News · L’Economiste (Économie) · EcoActu (Éco Nat.) · Médias24 (LeBoursier)
...
• Proxy automático para dominios que devuelven 403 (Medias24)
"""
...
def _session() -> requests.Session:
    ...
    s.headers.update({
        "User-Agent": (
            "Mozilla/5.0 (compatible; MoroccanFinanceBot/1.4; "
...
# dominios que consultaremos vía proxy
_PROXY = {"medias24.com": lambda u: "https://r.jina.ai/http://" + u.removeprefix("https://").removeprefix("http://")}

def fetch(url: str, timeout: float = 10.0) -> str:
    """
    Descarga la URL; si el dominio está en _PROXY la pide vía proxy.
    """
    host = urllib.parse.urlparse(url).netloc
    proxied = _PROXY.get(host, lambda x: x)(url)
    r = _session().get(proxied, timeout=timeout)
    r.raise_for_status()
    return r.text
...
def _send_telegram(head: str, desc: str, link: str, img: str | None):
    ...
    if img:
        try:
            if (r_head := requests.head(img, timeout=5)).ok and r_head.headers.get("Content-Type", "").startswith("image/"):
            ...
            print("[WARN] sendPhoto falló – texto simple:", e)
...
def _extract_first(block: BeautifulSoup, specs: str, base_url: str) -> str:
    for spec in [s.strip() for s in specs.split(",") if s.strip()]:
        if "::attr(" in spec:
            ...
            if (tag := block.select_one(css)) and tag.has_attr(attr):
                raw = tag[attr]
                if attr == "style" and "background-image" in raw:
                    if (m := re.search(r'url\((["\']?)(.*?)\1\)', raw)):
                        raw = m.group(2)
                return urljoin(base_url, raw)
        else:
            if (t := block.select_one(spec)) and t.has_attr("src"):
                return urljoin(base_url, t["src"])
    return ""
...
def _parse(src: Dict) -> List[Dict]:
    ...
    for bloc in soup.select(sel["container"]):
        if not (a := bloc.select_one(sel["headline"])): continue
        ...
        if sel.get("description"):
            if (d := bloc.select_one(sel["description"])): desc = d.get_text(strip=True)

        img = _extract_first(bloc, sel.get("image", ""), src["base_url"]) if sel.get("image") else ""

        raw_date = ""
        if sel.get("date"):
            if (d := bloc.select_one(sel["date"])): raw_date = d.get_text(strip=True)

        parsed = ""
        if (rx := src.get("date_regex")) and raw_date and (m := re.search(rx, raw_date)):
            if src.get("month_map"):
                d, mon, y = m.groups()
                if (mm := src["month_map"].get(mon)): parsed = f"{y}-{mm}-{int(d):02d}"
            else:
...
def main():
    ...
    cache  = _load_cache()
    ACTIVE = {"medias24_leboursier"}  # solo estamos probando esta fuente
